...
# Role: Extract the icon from a given executable file as a base-64 PNG string.
# Args:
...
# Name: 'PrintCSV'
# Role: Print application names, executable paths and base-64 encoded icons as semicolon-seperated CSV ($id;$name;$path;$icon).
# Args:
...
function PrintCSV
{
    ...
    foreach ($Application in $NamesandPaths)
    {
        ...
        if ($Source -eq "choco")
        {
            if ( $Name.EndsWith(" - Chocolatey Shim"))
            {
                $Name = $Name.Substring(0, $Name.Length - " - Chocolatey Shim".Length)
            }
        }
        ...
        if ($Source -ne "winreg")
        {
            $Name = $Name + " [" + $Source.ToUpper() + "]"
        }

        # Store the application icon as a base-64 string.
        $Icon = GetApplicationIcon -exePath $Application.Path

        # Output the results as a CSV row
        Write-Output "$($Application.Name.Replace(' ', '').ToLower());$Name;$($Application.Path);$Icon"
    }
}
...
function AppSearchWinReg
{
    ...
    PrintCSV -Names $exeNames -Paths $validPaths -Source "winreg"
}
...
function AppSearchChocolatey
{
    ...
    if (Test-Path -Path $chocoDir -PathType Container)
    {
        ...
        # Process extracted executable file paths.
        PrintCSV -Names $exeNames -Paths $exePaths -Source "choco"
    }
}
...
function AppSearchScoop
{
    ...
    if (Test-Path -Path $scoopDir -PathType Container)
    {
        ...
        # Process extracted executable file paths.
        PrintCSV -Names $exeNames -Paths $exePaths -Source "scoop"
    }
}
